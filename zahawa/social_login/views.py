...
class CustomAppleSocialLoginSerializer(SocialLoginSerializer):
    ...
    def social_login(self, request, app, token, **kwargs):
    ...
    def validate(self, attrs):
        ...
        if attrs.get('access_token'):
            ...
            token = {'access_token': access_token,"id_token":access_token}
        ...
        if not login.is_existing:
            ...
            if allauth_settings.UNIQUE_EMAIL:
                ...
                if get_user_model().objects.filter(email=login.user.email).exists():
                    raise serializers.ValidationError(('E-mail already registered using different signup method.'))
